...
```bash
curl "https://v2.jkt48connect.my.id/api/jkt48/recent/detail/{data_id}?apikey=YOUR_API_KEY"
```
</Step>
</Steps>
...
**Endpoint:** `/api/jkt48/recent/detail/{data_id}`  
**Method:** `GET`  
...
**Example:**
```http
GET /api/jkt48/recent/detail/125100001751127046?apikey=YOUR_API_KEY HTTP/1.1
...
async function getStreamDetail(dataId) {
  try {
    const response = await fetch(
      `${BASE_URL}/api/jkt48/recent/detail/${dataId}?apikey=${API_KEY}`
    );
...
def get_stream_detail(data_id):
    ...
    url = f"{BASE_URL}/api/jkt48/recent/detail/{data_id}"
...
func getStreamDetail(dataID string) (*StreamDetail, error) {
    url := fmt.Sprintf("%s/api/jkt48/recent/detail/%s?apikey=%s", BaseURL, dataID, APIKey)
...
3. **Fetch Details**: Use the `data_id` with `/api/jkt48/recent/detail/{data_id}`
